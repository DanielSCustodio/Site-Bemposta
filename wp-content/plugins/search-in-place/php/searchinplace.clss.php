...
class CodePeopleSearchInPlace {
	...
	private function _process_summary(&$text, $terms, $length, $post_id)
	{
		...
		$p = ($i !== false && $i-$length/2 > 0) ? $i-$length/2 : 0;
	...
	*/
	function init(){
		...
		if(!empty($search_in_page_selectors)) $this->javascriptVariables['areas'] = $search_in_page_selectors;
	...
	*/
	public function get_search_form($atts = array())
	{
		...
		if(isset($atts['display_button']) && @intval($atts['display_button']))
			$search = str_replace('name="s"', 'name="s" data-display-button="1"', $search);

		$search = preg_replace('/<\/form>/i', '<input type="hidden" name="search_in_place_form" value="1"></form>', $search);
		$search = apply_filters('search-in-page-form', $search);
		return $search;
	} // End get_search_form
    ...
    public function populate() {
        ...
        foreach($posts as $result){
			...
			if(get_option('search_in_place_display_summary', 1)){
				...
				if(!$in_excerpt) // The term is not in the excerpt
				{
	...
	*/
	function printAdminPage(){
		...
		if(isset($_POST['search_in_place_submit']) && !isset($_POST['search_in_place_delete_settings']))
		{
			...
			$search_in_page_selectors = (isset($_POST['search_in_page_selectors'])) ? sanitize_text_field($_POST['search_in_page_selectors']) : '';
			$search_in_page_selectors = stripslashes($search_in_page_selectors);
			$search_in_page_selectors = explode(",", $search_in_page_selectors);
			foreach($search_in_page_selectors as $key => $value)
			{
				if(trim($value) == '') unset($search_in_page_selectors[$key]);
			}
				...
				<form method="post" action="'.sanitize_text_field($_SERVER['REQUEST_URI']).'">
					...
					<table class="form-table">
						<tbody>
							...
							<tr valign="top">
								...
								<td>
									<input aria-label="Connection operator - or" type="radio" name="connection_operator" value="or" '.( ( $search_in_place_connection_operator == 'or' ) ? 'CHECKED' : '' ).' /> OR&nbsp;&nbsp;&nbsp;&nbsp;
									<input aria-label="Connection operator - and" type="radio" name="connection_operator" value="and" '.( ( $search_in_place_connection_operator == 'and' ) ? 'CHECKED' : '' ).' /> AND <br />
					...
					<table class="form-table">
						<tbody>
							<tr valign="top">
								<td>
									<input aria-label="Post title" type="checkbox" checked disabled name="title" id="title"> '.__('Post title', $this->text_domain).' <input aria-label="Post thumbnail" type="checkbox" name="thumbnail" id="thumbnail" value="1" '.(($search_in_place_display_thumbnail == 1) ? 'checked' : '').' /> '.__('Post thumbnail', $this->text_domain).' <input aria-label="Post author" type="checkbox" name="author" value="1" id="author" '.(($search_in_place_display_author == 1) ? 'checked' : '').' /> '.__('Post author', $this->text_domain).' <input aria-label="Post date" type="checkbox" name="date" id="date" value="1" '.(($search_in_place_display_date == 1) ? 'checked' : '').' /> '.__('Post date', $this->text_domain).' <input aria-label="Post summary" type="checkbox" name="summary" id="summary" value="1" '.(($search_in_place_display_summary == 1) ? 'checked' : '').' /> '.__('Post summary', $this->text_domain).'
								</td>
							</tr>
						</tbody>
					</table>
					...
					<table class="form-table">
						<tbody>
							...
							<tr valign="top">
								<th scope="row">
									<label for="highlight">'.__("Search in page selectors", $this->text_domain).'</label>
								</th>
								<td>
									...
									<div><i>'.__('Enter the selector where to search and highlight the search terms separated by comma.<br>Ex. <b>div.hentry,#content,#main,div.content,#middle,#container,#wrapper,article,.elementor,body</b>', $this->text_domain).'</i></div>
								</td>
							</tr>
						</tbody>
					</table>';
						...
						<table class="form-table">
							<tbody>
								<tr valign="top">
									...
									<td>
										<input aria-label="Delete all settings" type="checkbox" name="search_in_place_delete_settings" id="search_in_place_delete_settings" /><br>
