...
if (!class_exists('BF_Admin_Page_Class_Pro')):
    ...
    class BF_Admin_Page_Class_Pro
    {
        ...
        public function __construct($args)
        {
            ...
            add_action('wp_ajax_apc_delete_mupload', array(
                $this,
                'wp_ajax_delete_image'
            ));

            //import export
            add_action('wp_ajax_apc_import_' . $this->option_group, array(
                $this,
                'import'
            ));

            add_action('wp_ajax_apc_export_' . $this->option_group, array(
                $this,
                'export'
            ));

            add_action('wp_ajax_apc_migration_' . $this->option_group, array(
                $this,
                'clt_migration_handler'
            ));

            //plupload ajax
            add_action('wp_ajax_plupload_action', array(
                $this,
                "Handle_plupload_action"
            ));
        ...
        public function AddMenuTopPage()
        {
            ...
            $id = add_menu_page($this->args['page_title'], $this->args['page_title'], $this->args['capability'], $this->args['id'], array(
                $this,
                'DisplayPage'
            ) , $this->args['icon_url'], $this->args['position']);
            $page = add_submenu_page($id, $this->args['page_title'], $this->args['page_title'], $this->args['capability'], $this->args['id'], array(
                $this,
                'DisplayPage'
            ));
        ...
        public function AddMenuSubPage()
        {
            ...
            $this->args = array_merge($default, $this->args);
          //  $subPage = 'edit.php?post_type=' . $this->Top_Slug;
            $page = add_submenu_page($this->Top_Slug, $this->args['page_title'], $this->args['page_title'], $this->args['capability'], $this->createSlug() , array(
                $this,
                'DisplayPage'
            ));
            if ($page)
        ...
        public function Load_page_hooker()
        {
            ...
            //panel script
            add_action('admin_footer-' . $page, array(
                $this,
                'panel_script'
            ));
...
?>
        ...
        */
        ...
        <div class="ctl_logo">' . "";
            ...
            echo '<img  style="width:50%;height:50%;margin:0;padding:0;" class="timeline_logo" src="' . CTP_PLUGIN_URL . 'assets/images/cool-timeline-logo.png">' . "\n" . ((isset($this->args['page_header_text'])) ? $this->args['page_header_text'] : '') . '
      ...
      <br style="clear:both"><br>
            ...
            foreach ($this->_fields as $field)
            {
                ...
                if (method_exists($this, 'show_field_' . $field['type']))
                {
                    ...
                    {
                        echo apply_filters('admin_page_class_field_container_open', '<div class="field">' . "\n", $field);
                    }
                    ...
                    {
                        echo apply_filters('admin_page_class_field_container_close', '</div>' . "\n", $field);
                    }
  ...
  <h2 class="ctl_add_head">Please Share Your Valuable Feedback.</h2>
  <p>Thanks for using <strong>Cool Timeline PRO</strong> WordPress plugin. We hope it meets your expectations!
  <br> Please give us a <strong>quick rating</strong>, it works as a boost for us to keep working on more cool plugins!</p>
<p>Here’s the link:-</p>
  ...
  <div><a target="_blank" href="https://codecanyon.net/item/cool-timeline-pro-wordpress-responsive-timeline-plugin/reviews/17046256"><img src="https://res.cloudinary.com/cooltimeline/image/upload/v1504097450/stars5_gtc1rg.png"></a>
    ...
    <img src="https://res.cloudinary.com/cooltimeline/image/upload/v1503490189/website-images/cool-timeline-demos.png"></a> <a target="_blank" href="http://www.cooltimeline.com/documentation"><img src="https://res.cloudinary.com/cooltimeline/image/upload/v1505133065/view-docs_gudqwz.png"></a></div> </div>';
        ...
        public function load_scripts_styles()
        {
            ...
            wp_enqueue_script('iphone_checkbox', $plugin_path . '/js/FancyCheckbox/FancyCheckbox.js', array(
                'jquery'
            ) , null, true);
        ...
        public function check_field_code()
        {
            ...
            if ($this->has_field('code') && $this->is_edit_page())
            {
                ...
                wp_enqueue_script('at-code-js', $plugin_path . '/js/codemirror/codemirror.js', array(
                    'jquery'
                ) , false, true);

                wp_enqueue_script('at-code-js-xml', $plugin_path . '/js/codemirror/xml.js', array(
                    'jquery'
                ) , false, true);

                wp_enqueue_script('at-code-js-javascript', $plugin_path . '/js/codemirror/javascript.js', array(
                    'jquery'
                ) , false, true);

                wp_enqueue_script('at-code-js-css', $plugin_path . '/js/codemirror/css.js', array(
                    'jquery'
                ) , false, true);

                wp_enqueue_script('at-code-js-clike', $plugin_path . '/js/codemirror/clike.js', array(
                    'jquery'
                ) , false, true);

                wp_enqueue_script('at-code-js-php', $plugin_path . '/js/codemirror/php.js', array(
                    'jquery'
                ) , false, true);
        ...
        public function check_field_select()
        {
            ...
            )) && $this->is_edit_page())
            {
                ...
                wp_enqueue_script('at-multiselect-chosen-js', $plugin_path . '/js/select2/select2.js', array(
                    'jquery'
                ) , false, true);
        ...
        public function check_field_plupload()
        {
            ...
            if ($this->has_field('plupload') && $this->is_edit_page())
            {
                ...
                wp_register_script('myplupload', $plugin_path . '/js/plupload/myplupload.js', array(
                    'jquery'
                ));
        ...
        public function check_field_upload()
        {
            ...
            add_filter('media_upload_gallery', array(
                $this,
                'insert_images'
            ));

            add_filter('media_upload_library', array(
                $this,
                'insert_images'
            ));

            add_filter('media_upload_image', array(
                $this,
                'insert_images'
            ));

            // Delete all attachments when delete custom post type.
            add_action('wp_ajax_at_delete_file', array(
                $this,
                'delete_file'
            ));

            add_action('wp_ajax_at_reorder_images', array(
                $this,
                'reorder_images'
            ));

            // Delete file via Ajax
            add_action('wp_ajax_at_delete_mupload', array(
                $this,
                'wp_ajax_delete_image'
            ));
        ...
        public function wp_ajax_delete_image()
        {
            ...
            else
            {
                ...
                if (isset($f[2]) && isset($saved[$f[1]][$f[2]]))
                { //delete from repeater  block
                    unset($saved[$f[1]][$f[2]]);
                ...
                elseif (isset($saved[$f[1]]['src']))
                { //delete from conditional block
                    unset($saved[$f[1]]);
            ...
            else
            {
                ...
                echo json_encode(array(
                    'message' => __('Cannot delete file. Something\'s wrong.', 'cool-timeline')
                ));
        ...
        public function show_field_repeater($field, $meta)
        {
            ...
            if (count($meta) > 0 && is_array($meta))
            {
                ...
                foreach ($meta as $me)
                {
                    ...
                    $mmm = (in_array($field['fields'][0]['type'], array(
                        'image',
                        'file'
                    )) ? '' : $mmm);
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
        ...
        public function show_field_wysiwyg($field, $meta)
        {
            ...
            else
            {
                ...
                wp_editor(stripslashes(stripslashes(html_entity_decode($meta))) , $field['id'], array(
                    'editor_class' => 'at-wysiwyg' . (isset($field['class']) ? " {$field['class']}" : "")
                ));
        ...
        public function save_field($field, $old, $new)
        {
            ...
            if ($new === '' || $new === array() && (!in_array($field['type'], array(
                'text',
                'textarea'
            ))))
        ...
        */
        ...
        public function add_missed_values()
        {
            ...
            foreach ($this->_fields as & $field)
            {
                ...
                $multiple = in_array($field['type'], array(
                    'checkbox_list',
                    'file',
                    'image'
                ));
        ...
        public function get_fonts_family($font = null)
        {
            ...
            if ($fonts === false)
            {
                ...
                if ($this->google_fonts)
                {
                    ...
                    $gs = wp_remote_get('https://www.googleapis.com/webfonts/v1/webfonts?sort=popularity&key=' . $api_keys[$k], array(
                        'sslverify' => false
                    ));
    ...
    ';
        ...
        <div style="padding 10px;" class="apc_export">
         <div class="export_code">';
            ...
            {
                $ret .= '<p>' . __('Migrate Stories From Free To Pro Version', 'cool-timeline2') . '</p><input class="button" type="button" value="' . __('Migrate content', 'cool-timeline') . '" id="apc_migration_b" />';
            }
            else
            {
                $ret .= '<label for="">' . __('No Stories is Available For Migration', 'cool-timeline2') . '</label>';
            }
    ...
    <input type="hidden" id="ctl_can_migrate" value="' . $ctl_can_migrate . '" />
      <input type="hidden" id="apc_migration_nonce" name="apc_migration" value="' . wp_create_nonce("apc_migration") . '" />';
        ...
        // custom ajax based content migration
